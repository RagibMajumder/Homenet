...
export function PropertyCard({
  ...
  const sellDisabled = !wallet.connected || !wallet.publicKey || isPending;
  ...
  async function handleSell() {
    if (!wallet.connected || !wallet.publicKey || !wallet.signMessage) {
      toast.error("Wallet cannot sign messages", {
        description: "Connect a wallet that supports message signing.",
      });
      return;
    }
    ...
    try {
      const message = new TextEncoder().encode(
        "Liquidate Homenet Asset: " + property.address,
      );
      await wallet.signMessage(message);

      toast.success("Asset Liquidated. USDC transferred to wallet.", {
        description: property.address,
      });

      setIsOwned(false);

      // Optional: notify the RWA terminal to log liquidation.
      window.dispatchEvent(
        new CustomEvent("homenet:rwa-liquidation", {
          detail: { address: property.address, time: nowTime() },
        }),
      );
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setError(msg);
      toast.error("Liquidation signature failed", { description: msg });
    } finally {
      setIsPending(false);
    }
  }
  ...
  return (
    <div className="group flex flex-col h-full rounded-2xl border border-white/10 bg-white/[0.06] p-4 shadow-sm backdrop-blur">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          {!isOwned ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isPending ? "Liquidating…" : "Sell Position"}
              <ArrowUpRight className="h-4 w-4" />
            </button>
          )}
        </div>
